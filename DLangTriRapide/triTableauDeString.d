...
class AvecChaineDeString
{
    ...
    void tri_rapide(string[] t)
    {
        quicksort_dernier_elt(t, 0, cast(int)(t.length - 1));
    }

    //Lomuto partition scheme
    int partition_dernier_elt(string[] A, int lo, int hi)
    {
        auto pivot = A[hi];
        int i = lo;
        for (int j = lo; j < hi; j++)
        {
            if (strcmp3(A[j], pivot) < 0) //ordre croissant
            {
                swap(A[i], A[j]);
                nb_permutations++;
                i = i + 1;
    ...
    void quicksort_dernier_elt(string[] A, int lo, int hi)
    {
        if (lo < hi)
        {
    ...
    static int strcmp3(string a, string b)
    {
